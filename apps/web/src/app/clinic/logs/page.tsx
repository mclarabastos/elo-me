...
export default async function ClinicLogsPage() {
  ...
  const institutionalEvents = auditTimeline.items.filter((item) =>
    ["access_request", "consent", "cre_validation", "audit_log"].includes(
      item.type
    )
  );

  const accessRequestEvents = institutionalEvents.filter(
    (item) => item.type === "access_request"
  );

  const validationEvents = institutionalEvents.filter(
    (item) => item.type === "cre_validation"
  );

  const authorizedEvents = institutionalEvents.filter(
    ...
    <div className="space-y-[18px]">
      <section>
        ...
        <p className="mt-3 max-w-[760px] text-[15px] leading-[1.55] text-[var(--ink-60)]">
          Acompanhe a trilha institucional do fluxo de acesso: solicitação,
          consentimento, validação e registro autorizado.
        </p>
      </section>
      ...
      <section className="grid gap-[14px] lg:grid-cols-3">
        <MetricCard
          label="Eventos"
          value={institutionalEvents.length}
        ...
        <MetricCard
          ...
          helper="acessos liberados no fluxo"
        />
      </section>
        ...
        <div className="flex flex-col justify-between gap-4 border-b border-[var(--line)] p-5 md:flex-row md:items-start">
          <div>
            ...
            <h2 className="mt-2 text-[22px] font-bold tracking-[-0.02em] text-[var(--navy)]">
              Histórico institucionais do acesso
            </h2>

            <p className="mt-2 max-w-[740px] text-[14px] leading-[1.55] text-[var(--ink-60)]">
              Esta visualização mostra eventos ligados à clínica no fluxo de
              atendimento. A auditoria completa dos dados pertence ao paciente.
            </p>
          </div>
        </div>
